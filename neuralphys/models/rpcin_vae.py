    ...
    def __init__(self):
        ...
        if C.RIN.COOR_FEATURE:
            ...
            self.pos_merger = nn.Sequential(
                nn.Conv2d(self.in_feat_dim + self.po_feat_dim, self.in_feat_dim, kernel_size=3, stride=1, padding=1),
                nn.ReLU(inplace=True),
            )
        ...
        if C.RIN.MASK_LOSS_WEIGHT > 0:
            if self.use_conv_for_mask:
                self.mask_decoder = nn.Sequential(
                    nn.Conv2d(self.in_feat_dim, self.in_feat_dim, kernel_size=1),
                    nn.ReLU(inplace=True),
                    nn.Conv2d(self.in_feat_dim, self.in_feat_dim, kernel_size=3, padding=1),
                    nn.ReLU(inplace=True),
                    nn.UpsamplingNearest2d(size=(C.RIN.MASK_SIZE, C.RIN.MASK_SIZE)),
                    nn.Conv2d(self.in_feat_dim, self.in_feat_dim, kernel_size=3, padding=1),
                    nn.ReLU(inplace=True),
    ...
    def forward(self, x, rois, p_feat, num_rollouts=10, g_idx=None, x_t=None, phase='train'):
        ...
        for i in range(num_rollouts):
            ...
            if C.RIN.MASK_LOSS_WEIGHT:
                if self.use_conv_for_mask:
                    mask = self.mask_decoder(s.reshape((-1,) + s.shape[2:]))
                    mask = mask.reshape((batch_size, -1) + mask.shape[2:])
                else:
        ...
        if len(mask_rollout) > 0:
            if self.use_conv_for_mask:
                mask_rollout = torch.stack(mask_rollout).transpose(1, 0)
            else:

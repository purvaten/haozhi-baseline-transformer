...
def bounce_n(T=128, n=2, r=None, m=None):
    ...
    for t in range(T):
        ...
        key_frame_t = [False for _ in range(n)]
        col_frame_t = False
            ...
            for i in range(n):
                for z in range(2):
                    if x[i][z] - r[i] < 0:
                        ...
                        key_frame_t[i] = True and abs(v[i][z]) > eps * 2
                    if x[i][z] + r[i] > size[z] - 1:
        ...
        v *= friction
        key_frame.append(key_frame_t)
        col_frame.append(col_frame_t)

    key_frame = np.array(key_frame)
    col_frame = np.array(col_frame)
    return X, y, init_a, key_frame, col_frame
...
def bounce_vec(res, n=2, T=128, r=None, m=None):
    ...
    while not valid:
        ...
        # pre_valid = (np.absolute(y)[:30].sum((0, 2)) > 1e-3).sum() < 3
        # valid = (np.absolute(y)[30:60].sum((0, 2)) > 1e-3).sum() == 3
...
if __name__ == "__main__":
    cache_name = 'data/dynamics/rinv0-r2-h64w64-v2-6'
